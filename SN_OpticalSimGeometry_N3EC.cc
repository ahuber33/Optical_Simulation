...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  LogicalN3EC_Scintillator = theScint->GetN3EC_Scintillator();
  LogicalN3EC_Teflon = theScint->GetN3EC_Teflon();
  LogicalN3EC_Mylar = theScint->GetN3EC_Mylar();
  
  // Set colors of various block materials
  LogicalN3EC_Scintillator->SetVisAttributes(cyan); // Set scint color to cyan
  LogicalN3EC_Teflon->SetVisAttributes(white); // Set teflon color to white
  LogicalN3EC_Mylar->SetVisAttributes(gray); // Set mylar color to gray 
  ...
  LogicalN3EC_LightGuide = theScint->GetN3EC_LightGuide();
  LogicalN3EC_WrapLightGuide = theScint->GetN3EC_WrapLightGuide();
  LogicalN3EC_InterfaceLightGuide = theScint->GetN3EC_InterfaceLightGuide();
  LogicalN3EC_WrapInterfaceLightGuide = theScint->GetN3EC_WrapInterfaceLightGuide();

  // Set colors of various light guide materials
  LogicalN3EC_LightGuide->SetVisAttributes(green); // Set light guide color to green
  LogicalN3EC_WrapLightGuide->SetVisAttributes(gray); // Set wrap light guide to gray (mylar)
  LogicalN3EC_InterfaceLightGuide->SetVisAttributes(green); // Set interface light guide to green 
  LogicalN3EC_WrapInterfaceLightGuide->SetVisAttributes(gray); // Set wrap interface light guide to gray (mylar)
  ...
  //OpticalTeflon->SetSigmaAlpha(0.01);
  ...
  SSTeflon = new G4LogicalSkinSurface("b_Teflon",LogicalN3EC_Teflon,OpticalTeflon); // Applies optical Teflon properties to ogical Teflon surface
  ...
  SSMylar = new G4LogicalSkinSurface("SSMylar",LogicalN3EC_Mylar,OpticalMylar); // Applies Mylar properties to logical mylar surface
  SSWrapLightGuide = new G4LogicalSkinSurface("SSWrapLightGuide",LogicalN3EC_WrapLightGuide,OpticalMylar); // Wraps light guide in Mylar
  SSWrapInterfaceLightGuide = new G4LogicalSkinSurface("SSWrapInterfaceLightGuide",LogicalN3EC_WrapInterfaceLightGuide,OpticalMylar); // Wraps interface light guide in Mylar
  ...
  LogicalN3EC_CouplingSc = CouplingGlue->GetN3EC_CouplingLightGuide(); // Call function for building coupling
  LogicalN3EC_CouplingPMMA = CouplingGlue->GetN3EC_CouplingLightGuide(); // Call function for building coupling (same function because same geometry but different name)
  LogicalN3EC_Glue = CouplingGlue->GetN3EC_SphericalCoupling(); // Call function for building glue
  LogicalN3EC_WrapGluedLightGuide = CouplingGlue->GetN3EC_WrapGluedLightGuide(); // Call function for building glued wrap around the light guide !!!! Be careful => Comment if you want Air gap
 LogicalN3EC_WrapGluedInterfaceLightGuide = CouplingGlue->GetN3EC_WrapGluedInterfaceLightGuide();

  LogicalN3EC_CouplingSc->SetVisAttributes(red); // Set glue color to red 
  LogicalN3EC_CouplingPMMA->SetVisAttributes(red); // Set glue color to red 
  LogicalN3EC_Glue->SetVisAttributes(red); // Set glue color to red 
  LogicalN3EC_WrapGluedLightGuide->SetVisAttributes(magenta);
  LogicalN3EC_WrapGluedInterfaceLightGuide->SetVisAttributes(magenta);
  ...
  G4String PMTfile = path+"QE_ham_GA0154.txt"; //File for 8 inches !!!
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      PMTdetect[PMTentries] = 0.67*quant_eff*.655;  //0.67 = correction factor to adjust QE to lower value of ~25-26 % with file from 8inches 0.643
  ...
  G4double CouplingSc_Placement = N3EC_ScintHeight/2 + N3EC_LightGuideCouplingHeight/2;
  G4double LightGuide_Placement = CouplingSc_Placement + N3EC_LightGuideCouplingHeight/2 + N3EC_LightGuideStepHeight + N3EC_LightGuideHeight/2;
  G4double CouplingPMMA_Placement = LightGuide_Placement + N3EC_LightGuideHeight/2 + N3EC_LightGuideCouplingHeight/2;
  G4double InterfaceLightGuide_Placement = CouplingPMMA_Placement + N3EC_LightGuideCouplingHeight/2 + N3EC_InterfaceLightGuideHeight/2;
  G4double Glue_Placement = InterfaceLightGuide_Placement + N3EC_InterfaceLightGuideHeight/2 + N3EC_CouplingSphereRadius - N3EC_CouplingSphereDepth;
  G4double PMT_Placement = InterfaceLightGuide_Placement + N3EC_InterfaceLightGuideHeight/2 - N3EC_CouplingSphereDepth + GlueThickness + PMTGlassRadius;
  
  G4cout << "CouplingSc_Placement = " << CouplingSc_Placement
        << "\nLightGuide_Placement = " << LightGuide_Placement
        << "\nCouplingPMMA_Placement = " << CouplingPMMA_Placement
        << "\nInterfaceLightGuide_Placement = " << InterfaceLightGuide_Placement
  ...
  PhysicalScint = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalN3EC_Scintillator,"PhysicalScint",
  ...
  PhysicalTeflon = new G4PVPlacement(G4Transform3D 
				     ...
				     LogicalN3EC_Teflon,"PhysicalTeflon",
				     LogicalHolder,false,0);  
  ...
  PhysicalMylar = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalN3EC_Mylar,"PhysicalMylar",
				    LogicalHolder,false,0);  


  // Coupling Sc/Light Guide placement
  PhysicalCouplingSc = new G4PVPlacement(G4Transform3D
				   (Flip,G4ThreeVector(0,0,translation_visu + CouplingSc_Placement)),
				   LogicalN3EC_CouplingSc,"PhysicalCouplingSc",
				   LogicalHolder,false,0);


  // Coupling Light Guide/Interface Light Guide placement
  PhysicalCouplingPMMA = new G4PVPlacement(G4Transform3D
				   (Flip,G4ThreeVector(0,0,translation_visu + CouplingPMMA_Placement)),
				   LogicalN3EC_CouplingPMMA,"PhysicalCouplingPMMA",
				   LogicalHolder,false,0);
   
  // Light guide placement
  PhysicalLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + LightGuide_Placement)),
				  LogicalN3EC_LightGuide,"PhysicalLightGuide",
				  LogicalHolder,false,0);

  // Wrap Light guide placement
  PhysicalWrapLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + LightGuide_Placement)),
				  LogicalN3EC_WrapLightGuide,"PhysicalWrapLightGuide",
				  LogicalHolder,false,0);
/*
//#################################################################################################################
  // Wrap Glued Light guide placement     *****BE CAREFUL!!!!!!!!! Comment if you want Air gap*******
  PhysicalWrapGluedLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + LightGuide_Placement)),
				  LogicalN3EC_WrapGluedLightGuide,"PhysicalWrapGluedLightGuide",
				  LogicalHolder,false,0);

  PhysicalWrapGluedInterfaceLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + InterfaceLightGuide_Placement)),
				  LogicalN3EC_WrapGluedInterfaceLightGuide,"PhysicalWrapGluedInterfaceLightGuide",
				  LogicalHolder,false,0);

//##################################################################################################################
*/
  // Interface Light guide placement
  PhysicalInterfaceLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + InterfaceLightGuide_Placement)),
				  LogicalN3EC_InterfaceLightGuide,"PhysicalInterfaceLightGuide",
				  LogicalHolder,false,0);

  // Wrap Interface Light guide placement
  PhysicalWrapInterfaceLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + InterfaceLightGuide_Placement)),
				  LogicalN3EC_WrapInterfaceLightGuide,"PhysicalWrapInterfaceLightGuide",
				  LogicalHolder,false,0);
