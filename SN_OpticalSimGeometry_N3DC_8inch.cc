...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  LogicalN3DC_Scintillator = theScint->GetN3DC_8inch_Scintillator();
  LogicalN3DC_Teflon = theScint->GetN3DC_Teflon();
  LogicalN3DC_Mylar = theScint->GetN3DC_8inch_Mylar();
  ...
  LogicalN3DC_8InchesGlue = CouplingGlue->GetN3DC_8InchesSphericalCoupling(); // Call function for building glue

  LogicalN3DC_8InchesGlue->SetVisAttributes(red); // Set glue color to red 
  ...
  G4String PMTfile = path+"QE_ham_GA0205.txt"; //File for 8 inches !!!
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      PMTdetect[PMTentries] = quant_eff*.75;  //For the GAxxxx file with good QE and 0.65 for collection efficiency
  ...
  PhysicalGlue = new G4PVPlacement(G4Transform3D
				   (Flip,G4ThreeVector(0,0,translation_visu + Glue_Placement +12.5*mm)),
				   LogicalN3DC_8InchesGlue,"PhysicalGlue",
				   LogicalHolder,false,0);
