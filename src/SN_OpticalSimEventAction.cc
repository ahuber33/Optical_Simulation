...
void SN_OpticalSimEventAction::BeginOfEventAction(const G4Event* evt){
  ...
  Statsdeposited.E_compton.clear();
  Statsdeposited.Gamma_Interaction_X.clear();
  Statsdeposited.Gamma_Interaction_Y.clear();
  Statsdeposited.Gamma_Interaction_Z.clear();
  

  if (evt->GetEventID() %1000 ==0)
    {
      G4cout << "\r[" << evt->GetEventID() << "] events simulated" << std::flush;
    }
...
void SN_OpticalSimEventAction::EndOfEventAction(const G4Event* evt){
  ...
  Statistics.Deposit = Verif_E_tot;
  ...
  if (Statistics.Generated >10)
  {
      ...
      G4cout << ""  <<  G4endl;
      G4cout << "Count Scintillation = " << Statistics.Count_Scintillation << G4endl;
