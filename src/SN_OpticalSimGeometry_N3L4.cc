...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  // ***********************
  //Scint
  N3L4_ScintLength = theScint->GetN3L4_ScintLength();
  N3L4_ScintWidth = theScint->GetN3L4_ScintWidth();
  N3L4_ScintHeight = theScint->GetN3L4_ScintHeight();
  // Wrapping
  ...
  SNXW_CouplingSphereRadius = theScint->GetSNXW_CouplingSphereRadius(); // For the test, we use the same light guide from the XWall's configuration !!!!!
  // PMT
  ...
  // Light Guide ===> For the test, we use the same light guide from the XWall's configuration !!!!!
  ...
  LogicalN3L4_Scintillator = theScint->GetN3L4_Scintillator();
  LogicalN3L4_Teflon = theScint->GetN3L4_Teflon();
  LogicalN3L4_Mylar = theScint->GetN3L4_Mylar();
  
  // Set colors of various block materials
  LogicalN3L4_Scintillator->SetVisAttributes(cyan); // Set scint color to cyan
  LogicalN3L4_Teflon->SetVisAttributes(white); // Set teflon color to white
  LogicalN3L4_Mylar->SetVisAttributes(gray); // Set mylar color to gray 


  //*********************** 
  // Build light guide volume
  //***********************   
  // Simply calls functions from Scintillator class   ===> For the test, we use the same light guide from the XWall's configuration !!!!!
  LogicalSNXW_LightGuide = theScint->GetSNXW_LightGuide();
  LogicalSNXW_WrapLightGuide = theScint->GetSNXW_WrapLightGuide();


  // Set colors of various light guide materials
  LogicalSNXW_LightGuide->SetVisAttributes(green); // Set light guide color to green
  LogicalSNXW_WrapLightGuide->SetVisAttributes(gray); // Set wrap light guide to gray (mylar)
  ...
  SSTeflon = new G4LogicalSkinSurface("b_Teflon",LogicalN3L4_Teflon,OpticalTeflon); // Applies optical Teflon properties to ogical Teflon surface
  ...
  SSMylar = new G4LogicalSkinSurface("SSMylar",LogicalN3L4_Mylar,OpticalMylar); // Applies Mylar properties to logical mylar surface
  SSWrapLightGuide = new G4LogicalSkinSurface("SSWrapLightGuide",LogicalSNXW_WrapLightGuide,OpticalMylar); // Wraps light guide in Mylar
  ...
  //*********************** 
  // Build coupling medium ===> For the test, we use the same light guide from the XWall's configuration so we can use the variables from SNXW !!!!!
  //*********************** 
  CouplingGlue = new Coupling();  // Initialize coupling class
  LogicalSNXW_CouplingSc = CouplingGlue->GetSNXW_CouplingLightGuide(); // Call function for building coupling
  LogicalSNXW_Glue = CouplingGlue->GetSNXW_SphericalCoupling(); // Call function for building glue
  LogicalSNXW_WrapGluedLightGuide = CouplingGlue->GetSNXW_WrapGluedLightGuide(); // Call function for building glued wrap around the light guide !!!! Be careful => Comment if you want Air gap


  LogicalSNXW_CouplingSc->SetVisAttributes(red); // Set glue color to red 
  LogicalSNXW_Glue->SetVisAttributes(red); // Set glue color to red 
  LogicalSNXW_WrapGluedLightGuide->SetVisAttributes(magenta);
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      //PMTdetect[PMTentries] = 0.61*quant_eff*.7;  //0.79 = correction factor to adjust QE to lower value of 34% for R6594 and 0.59 = 25%
      PMTdetect[PMTentries] = 0.67*quant_eff*.655;  //0.67 = correction factor to adjust QE to lower value of ~25-26 % with file from 8inches
  ...
  G4double CouplingSc_Placement = N3L4_ScintHeight/2 + SNXW_LightGuideCouplingHeight/2;
  G4double LightGuide_Placement = CouplingSc_Placement + SNXW_LightGuideCouplingHeight/2 + SNXW_LightGuideHeight/2;
  G4double Glue_Placement = LightGuide_Placement + SNXW_LightGuideHeight/2 + SNXW_CouplingSphereRadius - SNXW_CouplingSphereDepth;
  G4double PMT_Placement = LightGuide_Placement + SNXW_LightGuideHeight/2 - SNXW_CouplingSphereDepth + GlueThickness + PMTGlassRadius;
  ...
  PhysicalScint = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalN3L4_Scintillator,"PhysicalScint",
  ...
  PhysicalTeflon = new G4PVPlacement(G4Transform3D 
				     ...
				     LogicalN3L4_Teflon,"PhysicalTeflon",
				     LogicalHolder,false,0);  
  ...
  PhysicalMylar = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalN3L4_Mylar,"PhysicalMylar",
				    LogicalHolder,false,0);  
  ...
  PhysicalCouplingSc = new G4PVPlacement(G4Transform3D
				   ...
				   LogicalSNXW_CouplingSc,"PhysicalCouplingSc",
				   LogicalHolder,false,0);
  ...
  PhysicalLightGuide = new G4PVPlacement(G4Transform3D
				  ...
				  LogicalSNXW_LightGuide,"PhysicalLightGuide",
				  LogicalHolder,false,0);

  // Wrap Light guide placement
  PhysicalWrapLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + LightGuide_Placement)),
				  LogicalSNXW_WrapLightGuide,"PhysicalWrapLightGuide",
				  LogicalHolder,false,0);
/*
//#################################################################################################################
  ...
  PhysicalWrapGluedLightGuide = new G4PVPlacement(G4Transform3D
  			       	  (DontRotate,G4ThreeVector(0,0,translation_visu + LightGuide_Placement)),
				  LogicalSNXW_WrapGluedLightGuide,"PhysicalWrapGluedLightGuide",
				  LogicalHolder,false,0);
