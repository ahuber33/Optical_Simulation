...
void SN_OpticalSimSteppingAction::UserSteppingAction(const G4Step *aStep){
  ...
  */
  ...
  if(fpSteppingManager->GetfN2ndariesPostStepDoIt()>2) {
 ...
 if(evtac->Getnph()==4)
  ...
  if ( partname =="e-" && aStep->GetPreStepPoint()->GetKineticEnergy()/keV >= 150 && aStep->GetTrack()->GetParentID() ==0) //150
    {
      //G4cout << " Track Length = " << aStep->GetTrack()->GetTrackLength()/mm << G4endl;
      //evtac->AddTrackLength(aStep->GetTrack()->GetStepLength()/mm);
  ...
  if((aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName()  == "PhysicalScint")
    ...
    {
      float Depth = -(-aStep->GetTrack()->GetPosition().z() -115.5);
      // G4cout << "z = " << aStep->GetTrack()->GetPosition().z() << G4endl;
      // G4cout << "z norm = " << -(aStep->GetTrack()->GetPosition().z() - 115.5) << G4endl;
      // G4cout << "Depth = " << Depth << G4endl;
      if (Depth > evtac->Get_Depth_Max())
	{
	  evtac->Set_Depth_Max(Depth);
	}
    }
