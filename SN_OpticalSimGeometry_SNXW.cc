...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  // ***********************
  //Scint
  SNXW_ScintWidth = theScint->GetSNXW_ScintWidth();
  SNXW_ScintHeight = theScint->GetSNXW_ScintHeight();
  // Wrapping
  ...
  LogicalSNXW_Scintillator = theScint->GetSNXW_Scintillator();
  LogicalSNXW_Teflon = theScint->GetSNXW_Teflon();
  LogicalSNXW_Mylar = theScint->GetSNXW_Mylar();
  
  // Set colors of various block materials
  LogicalSNXW_Scintillator->SetVisAttributes(cyan); // Set scint color to cyan
  LogicalSNXW_Teflon->SetVisAttributes(white); // Set teflon color to white
  LogicalSNXW_Mylar->SetVisAttributes(gray); // Set mylar color to gray 
  ...
  SSTeflon = new G4LogicalSkinSurface("b_Teflon",LogicalSNXW_Teflon,OpticalTeflon); // Applies optical Teflon properties to ogical Teflon surface
  ...
  SSMylar = new G4LogicalSkinSurface("SSMylar",LogicalSNXW_Mylar,OpticalMylar); // Applies Mylar properties to logical mylar surface
 ...
 // Build the PMT glass structure from PMT class
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      //PMTdetect[PMTentries] = 0.6136*quant_eff*.7;  //0.79 = correction factor to adjust QE to lower value of 34% for R6594 and 0.59 = 25% 0,6136
      PMTdetect[PMTentries] = 0.67*quant_eff*0.655;  //0.67 = correction factor to adjust QE to lower value of ~25-26 % with file from 8inches
  ...
  G4double CouplingSc_Placement = SNXW_ScintHeight/2 + SNXW_LightGuideCouplingHeight/2;
  ...
  PhysicalScint = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalSNXW_Scintillator,"PhysicalScint",
  ...
  PhysicalTeflon = new G4PVPlacement(G4Transform3D 
				     ...
				     LogicalSNXW_Teflon,"PhysicalTeflon",
				     LogicalHolder,false,0);  
  ...
  PhysicalMylar = new G4PVPlacement(G4Transform3D
				    ...
				    LogicalSNXW_Mylar,"PhysicalMylar",
				    LogicalHolder,false,0);  
