...
G4VPhysicalVolume* SN_OpticalSimGeometry::Construct( ){
  ...
  if (ReadPMT.is_open()){
    while(!ReadPMT.eof()){
      ...
      PMTdetect[PMTentries] = 0.67*quant_eff*.655;  //0.643...0.67 = correction factor to adjust QE to lower value of ~25-26 % with file from 8inches
  ...
  */
  ...
  PhysicalGlue = new G4PVPlacement(G4Transform3D
				   (Flip,G4ThreeVector(0,0,translation_visu+GlueZpos)),
				   LogicalSNMW_Glue,"PhysicalGlue",
				   LogicalHolder,false,0);

  // PMT placement
  PhysicalPMTGlass = new G4PVPlacement(G4Transform3D
				       (Flip,G4ThreeVector(0,0,translation_visu+GlueZpos+11.5*mm)),
